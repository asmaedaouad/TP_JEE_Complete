...
<%
    // Vérification de session - FORCER LA CONNEXION
...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-4">
        ...
        <div>
            ...
            <a href="${pageContext.request.contextPath}/produit?action=logout" class="btn btn-link btn-sm">Déconnexion</a>
        </div>
    </div>
    ...
    <c:if test="${sessionScope.role == 'ADMIN'}">
        <div class="border p-3 mb-4">
            ...
            <form action="${pageContext.request.contextPath}/produit" method="post">
                <input type="hidden" name="action" value="${not empty produitEdit ? 'update' : 'add'}">
                ...
                <div class="row g-2">
                    ...
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-primary w-100">
                            ${not empty produitEdit ? 'Modifier' : 'Ajouter'}
                        </button>
                    </div>
                </div>
            </form>
            ...
            <c:if test="${not empty produitEdit}">
                <div class="mt-2">
                    <a href="${pageContext.request.contextPath}/produit">Annuler</a>
                </div>
            </c:if>
        </div>
    </c:if>
    ...
    <!-- Recherche -->
    <div class="mb-3">
        <form action="${pageContext.request.contextPath}/produit" method="get" class="row g-2">
            <div class="col-auto">
            ...
            <div class="col-auto">
                <a href="${pageContext.request.contextPath}/produit" class="btn btn-link">Réinitialiser</a>
            </div>
        </form>
    </div>
                ...
                <c:otherwise>
                    <c:forEach var="p" items="${listeProduits}">
                        <tr>
                            ...
                            <c:if test="${sessionScope.role == 'ADMIN'}">
                                <td>
                                    <a href="${pageContext.request.contextPath}/produit?action=edit&id=${p.id}" class="btn btn-sm btn-outline-warning">Modifier</a>
                                    <a href="${pageContext.request.contextPath}/produit?action=delete&id=${p.id}" 
